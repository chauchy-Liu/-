...
name = algConfig['tatong_qingjiao']['name']#'风机基础不均匀沉降'
# 把所需测点定义到每个算法里
ai_points = algConfig['tatong_qingjiao']['ai_points']
ai_rename = algConfig['tatong_qingjiao']['ai_rename']
di_points = algConfig['tatong_qingjiao']['di_points']
general_points = algConfig['tatong_qingjiao']['general_points']
private_points = algConfig['tatong_qingjiao']['private_points']
# modelId = algConfig['tatong_qingjiao']['name']
time_duration = algConfig['tatong_qingjiao']['time_duration']
resample_interval = algConfig['tatong_qingjiao']['resample_interval']
error_data_time_duration = algConfig['tatong_qingjiao']['error_data_time_duration']
horizonTime = algConfig['tatong_qingjiao']['horizonTime']
need_all_turbines = algConfig['tatong_qingjiao']['need_all_turbines']
input_chunk_length = algConfig['tatong_qingjiao']['input_chunk_length']
out_chunck_length = algConfig['tatong_qingjiao']['out_chunck_length']
futureCovariates = algConfig['tatong_qingjiao']['futureCovariates']
numEpoch = algConfig['tatong_qingjiao']['numEpoch']
trainCutOff = algConfig['tatong_qingjiao']['trainCutOff']
timeSampleNum = algConfig['tatong_qingjiao']['timeSampleNum']
store_file = algConfig['tatong_qingjiao']['store_file']
timeUnit = resample_interval #'10T' #10minute: 10T,  1day:1D, 1quater:1Q
forcastHorizon = out_chunck_length #从预测点开始的预测数据长度
...
def ProcessSingleMeasurement(data, final_df, measureName, assetId, threshold, alarming, statement, Figs):
    ...
    #             break
        ...
        # Figs = []
        curves1 = []
        curves1.append(DisplayResultXY('0', algConfig['tatong_qingjiao']['nameMaps'][measureName], '#00FF00', 'Solid', data1))#{'type':'0', 'name':'倾角', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data1}
        ...
        result1 = DisplayFigures(xUnit=interval_unit, yUnit="度[°]", time=1, multiDimensionDataxy=curves1)#DisplayResultXY(str(final_df.index.min()), str(final_df.index.max()), str(interval_value), '角度', curves)
        Figs.append(result1)
        ...
        statement = str.replace(statement, measureName, algConfig['tatong_qingjiao']['nameMaps'][measureName])
        # 展示数据
        interval_value, interval_unit = time_util.split_time_delta(resample_interval) 
        interval_unit = time_util.__timedelta_resample_unit_dict[interval_unit].lower()
        ...
        curves1 = []
        curves1.append(DisplayResultXY('0', algConfig['tatong_qingjiao']['nameMaps'][measureName], '#00FF00', 'Solid', data1))#{'type':'0', 'name':'倾角', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data1}
        curves1.append(DisplayResultXY('0', str(algConfig['tatong_qingjiao']['nameMaps'][measureName])+'趋势', '#FFFF00', 'Solid', data2))#{'type':'0', 'name': '告警线', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data2}

        result1 = DisplayFigures(xUnit=interval_unit, yUnit="°", time=1, multiDimensionDataxy=curves1)#DisplayResultXY(str(final_df.index.min()), str(final_df.index.max()), str(interval_value), '角度', curves)
        Figs.append(result1)
        if warning == 0:
        ...
        # Figs = []
        curves1 = []
        curves1.append(DisplayResultXY('0', algConfig['tatong_qingjiao']['nameMaps'][measureName], '#FFFF00', 'Solid', data1))#{'type':'0', 'name':'倾角', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data1}
        curves1.append(DisplayResultXY('0', '告警线', '#FF0000', 'Solid', data2))#{'type':'0', 'name': '告警线', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data2}
...
async def judge_model(pn_data: DataFrame, Turbine_attr, threshold, idMaps,algorithms_config):
    ...
    if set(measureName).issubset(set(threshold["gaojing"]["threshold"].keys())) and not isinstance(measureName, str): #多测点
        ...
        for keyMeasure in measureName:
            #告警
            statementException = str(algConfig['tatong_qingjiao']['nameMaps'][keyMeasure])+f'超过阈值{threshold["gaojing"]["threshold"][keyMeasure]["10"]}，'
            statementNormal = str(algConfig['tatong_qingjiao']['nameMaps'][keyMeasure])+f'未超过阈值{threshold["gaojing"]["threshold"][keyMeasure]["10"]}, '
            ...
            data, statement, alarming =  alarm.generateAlarm(name, 'tatong_qingjiao', keyMeasure,final_df, error_data_time_duration, resample_interval, assetId, threshold, statementException, statementNormal, idMaps)
        ...
        #告警
        statementException = f'倾角超过阈值{threshold["gaojing"]["threshold"]["10"]}，'
        statementNormal = f'倾角未超过阈值{threshold["gaojing"]["threshold"]["10"]}'
        Figs = [] #存放每一张图
        final_df['result'] = final_df[measureName] > threshold["gaojing"]["threshold"]["10"]
        data, statement, alarming =  alarm.generateAlarm(name, 'tatong_qingjiao', 'TOAT',final_df, error_data_time_duration, resample_interval, assetId, threshold, statementException, statementNormal, idMaps)
