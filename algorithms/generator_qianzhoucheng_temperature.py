...
name = algConfig['generator_qianzhoucheng_temperature']['name']#'发电机驱动端轴承温度异常'
# 把所需测点定义到每个算法里
ai_points = algConfig['generator_qianzhoucheng_temperature']['ai_points']
ai_rename = algConfig['generator_qianzhoucheng_temperature']['ai_rename']
di_points = algConfig['generator_qianzhoucheng_temperature']['di_points']
general_points = algConfig['generator_qianzhoucheng_temperature']['general_points']
private_points = algConfig['generator_qianzhoucheng_temperature']['private_points']
time_duration = algConfig['generator_qianzhoucheng_temperature']['time_duration']
resample_interval = algConfig['generator_qianzhoucheng_temperature']['resample_interval'] # 原始数据采样间隔
error_data_time_duration = algConfig['generator_qianzhoucheng_temperature']['error_data_time_duration']
need_all_turbines = algConfig['generator_qianzhoucheng_temperature']['need_all_turbines']
...
def predict_result(pn_data: DataFrame, assetId):
    # 加载模型
    model = model_util.load_model(config.Wind_Farm, 'generator_qianzhoucheng_temperature', assetId,'generator_qianzhoucheng_temperature')
    # 预测健康温度 并判断是否告警
...
async def judge_model(pn_data: DataFrame, Turbine_attr, threshold, idMaps,algorithms_config):
    ...
    y1 = [str(round(num,4)) for num in list(pn_data['WGEN.TemGenDriEnd_predict'])]
    y2 = [str(round(num,4)) for num in list(pn_data['WGEN.TemGenDriEnd'])]
    ...
    # 生成告警
    data, statement, alarming =  alarm.generateAlarm(name, 'generator_qianzhoucheng_temperature', 'WGEN.TemGenDriEnd', pn_data, error_data_time_duration, resample_interval, assetId, threValue, statementException, statementNormal, idMaps)
    return data, statement, Figs, 0,1 # alarming, 0


def judge(pn_data: DataFrame):
    """
    发电机前轴承温度异常(发电机驱动端轴承温度异常)
    ...
    for index, row in pn_data.iterrows():
        ...
        c85 = float(row["WGEN.TemGenDriEnd"])

        if c85 >= 85 and c85 - c81 >= 45:
