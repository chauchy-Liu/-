...
name = algConfig['engine_env_temperature']['name']#'机舱环境温度异常'
# 把所需测点定义到每个算法里
ai_points = algConfig['engine_env_temperature']['ai_points']
ai_rename = algConfig['engine_env_temperature']['ai_rename']
di_points = algConfig['engine_env_temperature']['di_points']
general_points = algConfig['engine_env_temperature']['general_points']
private_points = algConfig['engine_env_temperature']['private_points']
time_duration = algConfig['engine_env_temperature']['time_duration']
resample_interval = algConfig['engine_env_temperature']['resample_interval']
error_data_time_duration = algConfig['engine_env_temperature']['error_data_time_duration']
need_all_turbines = algConfig['engine_env_temperature']['need_all_turbines']
store_file = algConfig['engine_env_temperature']['store_file']
...
def predict_result(pn_data: DataFrame):
    # 计算全场平均温度
    # mean_temperature = pn_data.groupby(pn_data.index)['WNAC.TemNacelle'].mean()
    value, unit = time_util.split_time_delta(resample_interval)
    unit = time_util.__timedelta_resample_unit_dict[unit].lower()
    mean_temperature = pn_data['WNAC.TemNacelle'].resample(str(6*value)+unit, closed='left').mean()
    #回填平均值
...
async def judge_model(pn_data: DataFrame, Turbine_attr, threshold, idMaps,algorithms_config):
    ...
    threValue = 100
    final_df['result'] = np.abs(target_temperature_df['WNAC.TemNacelle'] - mean_temperature) > threValue
    ...
    y2 = [str(round(num,4)) for num in list(pn_data['WNAC.TemNacelle'])]
    ...
    statementException = f'平均温度和风机实际温度偏差大于5'
    statementNormal = f'平均温度和风机实际温度偏差小于等于5'
    # 生成告警
    data, statement, alarming = alarm.generateAlarm(name, 'engine_env_temperature', 'WNAC.TemNacelle', final_df, error_data_time_duration, resample_interval, assetId, threValue, statementException,statementNormal, idMaps)
