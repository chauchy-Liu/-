...
name = algConfig['chilunxiang_disu_zhoucheng_temperature']['name']#'齿轮箱低速轴轴承温度异常'
# 把所需测点定义到每个算法里
ai_points = algConfig['chilunxiang_disu_zhoucheng_temperature']['ai_points']
ai_rename = algConfig['chilunxiang_disu_zhoucheng_temperature']['ai_rename']
di_points = algConfig['chilunxiang_disu_zhoucheng_temperature']['di_points']
general_points = algConfig['chilunxiang_disu_zhoucheng_temperature']['general_points']
private_points = algConfig['chilunxiang_disu_zhoucheng_temperature']['private_points']
time_duration = algConfig['chilunxiang_disu_zhoucheng_temperature']['time_duration']
resample_interval = algConfig['chilunxiang_disu_zhoucheng_temperature']['resample_interval']
error_data_time_duration = algConfig['chilunxiang_disu_zhoucheng_temperature']['error_data_time_duration']
need_all_turbines = algConfig['chilunxiang_disu_zhoucheng_temperature']['need_all_turbines']
store_file = algConfig['chilunxiang_disu_zhoucheng_temperature']['store_file']
...
def predict_result(pn_data: DataFrame, assetId):
    ...
    # 加载模型
    model = model_util.load_model(config.Wind_Farm, 'chilunxiang_disu_zhoucheng_temperature', assetId,'chilunxiang_disu_zhoucheng_temperature')
    # 预测健康温度 并判断是否告警
    y_predict = model.predict(pn_data[['WGEN.GenActivePW','WNAC.TemNacelle','WGEN.GenSpd']])
    return y_predict
...
async def judge_model(pn_data: DataFrame, Turbine_attr, threshold, idMaps,algorithms_config):
    ...
    # 模型推理
    # pn_data['WTRM.TemGeaLSND_predict'] = predict_result(pn_data, assetId)
    pn_data['WTRM.TemGeaLSND_predict'] = predict_result(pn_data, assetId)
    # 阈值判断
    threValue = 100
    pn_data['result'] = np.abs(pn_data['WTRM.TemGeaLSND'] - pn_data['WTRM.TemGeaLSND_predict']) > threValue
    ...
    y1 = [str(round(num,4)) for num in list(pn_data['WTRM.TemGeaLSND_predict'])]
    y2 = [str(round(num,4)) for num in list(pn_data['WTRM.TemGeaLSND'])]
    ...
    # 生成告警
    data, statement, alarming =  alarm.generateAlarm(name,'chilunxiang_disu_zhoucheng_temperature','WTRM.TemGeaLSND', pn_data, error_data_time_duration, resample_interval, assetId, threValue, statementException, statementNormal, idMaps)
...
def judge(pn_data: DataFrame):
    ...
    ？？齿轮箱中间轴驱动端轴承温度（WTRM.TemGeaZSDE）、齿轮箱中间轴非驱动端轴承温度（WTRM.TemGeaZSND）
