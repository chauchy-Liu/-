# -*- coding: utf-8 -*-
"""
Created on Tue Jul  9 11:29:48 2024
...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 25 17:43:43 2024
...
#########风场信息读取
# def WindFarm_attributes_Enos(AccessKey,SecretKey,assetIds,url):
#     WindFarm_attr = pd.DataFrame()
#     url_asset = str(str(url) + '&mdmIds=' + str(assetIds) + '&mdmTypes=EnOS_Wind_Farm&attributes=&locale=zh-CN')
    
#     ResponsePoint = poseidon.urlopen(AccessKey, SecretKey, url_asset)
#     #print(ResponsePoint)
#     if ResponsePoint['pagination']['pageSize'] > 0:
#         WindFarm_attr = pd.DataFrame(ResponsePoint['data'][assetIds]['mdmObjects']['EnOS_Wind_Farm'][:])
#     return WindFarm_attr

# #########风机信息读取
# def Turbine_attributes_Enos(AccessKey,SecretKey,assetIds,url):
#     Turbine_attr = pd.DataFrame()
#     url_asset = str(str(url) + '&mdmIds=' + str(assetIds) + '&mdmTypes=EnOS_Wind_Turbine&attributes=mdmId,name,ratedPower,rotorDiameter,turbineTypeID,altitude,hubHeight,cutOutwindSpeed,longitude,latitude')
...
# #########读取风机数据(原始)
# def ReadData_Enos_raw(startTime,SecretKey,AccessKey,assetIds,pointIds,url):
#     DfTemp = pd.DataFrame()
#     endTime = (datetime.datetime.strptime(startTime,"%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=0.1)).strftime("%Y-%m-%d %H:%M:%S")
#     params = {"Access Key":AccessKey,
#               "Secret Key":SecretKey,
#               "assetIds":assetIds,
#               "pointIds":pointIds,#pointIdsWithLogic
#               "startTime":startTime,
#               "endTime":endTime,
#               "itemFormat":"1",
#               "pageSize":"10000"}
...
# #Input_pointIds0 = "WNAC.WindSpeed,WGEN.GenActivePW,WROT.Blade1Position,WROT.Blade2Position,WROT.Blade3Position,WTRM.RotorPDM"
# #data_raw = ReadData_Enos_raw('2024-05-14 00:00:00',Input_SecretKey,Input_AccessKey,'rqT09vxy',Input_pointIds0,Input_url_data_raw)


# #########读取风机厂家状态数据
# def ReadState_CJ_Enos(startTime,SecretKey,AccessKey,assetIds,url):
#     StateTemp = pd.DataFrame()
#     endTime = (datetime.datetime.strptime(startTime,"%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=0.5)).strftime("%Y-%m-%d %H:%M:%S")
#     params = {"Access Key":AccessKey,
#               "Secret Key":SecretKey,
#               "assetIds":assetIds,
#               "pointIds":"WTUR.TurbineSts_Map,WTUR.TurbineSts",#pointIdsWithLogic
#               "startTime":startTime,
#               "endTime":endTime,
...
#     #print(ResponsePoint)
#     if len(ResponsePoint['data']['items']) > 0:
#         StateTemp = pd.DataFrame(ResponsePoint['data']['items'])
#     return StateTemp

# #########读取中台统一状态数据
# def ReadState_TY_Enos(startTime,SecretKey,AccessKey,assetIds,url):
#     StateTemp = pd.DataFrame()
#     endTime = (datetime.datetime.strptime(startTime,"%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=0.5)).strftime("%Y-%m-%d %H:%M:%S")
#     params = {"Access Key":AccessKey,
#               "Secret Key":SecretKey,
#               "assetIds":assetIds,
#               "pointIds":"WTUR.TurbineUnionSts",#pointIdsWithLogic
#               "startTime":startTime,
#               "endTime":endTime,
#               #"localTimeAccuracy":"true",
#               "itemFormat":"1",
...
# #########读取风机遥测状态数据
# def ReadState_Enos(startTime,SecretKey,AccessKey,assetIds,url):
#     StateTemp = pd.DataFrame()
#     endTime = (datetime.datetime.strptime(startTime,"%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=0.5)).strftime("%Y-%m-%d %H:%M:%S")
#     params = {"Access Key":AccessKey,
#               "Secret Key":SecretKey,
#               "assetIds":assetIds,
#               "pointIds":"WTUR.TurbineAIStatus",#pointIdsWithLogic
#               "startTime":startTime,
#               "endTime":endTime,
#               #"localTimeAccuracy":"true",
#               "itemFormat":"1",
...
# #########读取风机故障数据
# def ReadFault_Enos(startTime,SecretKey,AccessKey,assetIds,url):
#     FaultTemp = pd.DataFrame()
#     endTime = (datetime.datetime.strptime(startTime,"%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=0.5)).strftime("%Y-%m-%d %H:%M:%S")
#     params = {"Access Key":AccessKey,
#               "Secret Key":SecretKey,
#               "assetIds":assetIds,
#               "pointIds":"WTUR.AIStatusCode,WTUR.AIStatusCode_Map",#pointIdsWithLogic
#               "startTime":startTime,
#               "endTime":endTime,
#               #"localTimeAccuracy":"true",
#               "itemFormat":"1",
...
#     #print(ResponsePoint)
#     if len(ResponsePoint['data']['items']) > 0:
#         FaultTemp = pd.DataFrame(ResponsePoint['data']['items'])
#     return FaultTemp

# #########读取风机偏航状态数据
# def ReadYaw_Enos(startTime,SecretKey,AccessKey,assetIds,url):
#     StateTemp = pd.DataFrame()
#     endTime = (datetime.datetime.strptime(startTime,"%Y-%m-%d %H:%M:%S") + datetime.timedelta(days=0.5)).strftime("%Y-%m-%d %H:%M:%S")
#     params = {"Access Key":AccessKey,
#               "Secret Key":SecretKey,
#               "assetIds":assetIds,
#               "pointIds":"WYAW.YawCCWSts,WYAW.YawCWSts",#pointIdsWithLogic
#               "startTime":startTime,
#               "endTime":endTime,
#               #"localTimeAccuracy":"true",
#               "itemFormat":"1",
...
def pwratcurve_rho(data,windbin,num):
    pc_df_rho = pd.DataFrame()
    for i in range(len(windbin)):
        temp = data[(data['wspd','nanmean']>=windbin[i]-0.25) & (data['wspd','nanmean']<windbin[i]+0.25)]
        if len(temp) >= num:
            pc_df_rho.loc[i,'windbin'] = windbin[i]
            pc_df_rho.loc[i,'pwrat'] = np.mean(temp['pwrat','nanmean'])
            pc_df_rho.loc[i,'wspd_mean'] = np.mean(temp['wspd','nanmean'])
...
def monthdata(datatemp,altitude,path):#月数据分析！！！！！！！
    ...
    plt.close(fig)
    return str(path+'/'+'month.png'), month_data
...
def data_clear(data,power_rate,speed_max,speed_min,rotor,altitude,state):
    
    res_df = data[(data['statel']==90002)&(data['state']==state)]
    
    #########三一机组
    #res_df = data[(data['statel']==90002)&((data['state']==state)|(data['state']==120))]
    
    res_df = res_df[abs(res_df['wdir0'])<=35.0] 
    res_df = res_df[res_df['wspd']<=50.0]
    res_df = res_df[(res_df['pwrat']>0) & (res_df['pwrat']<power_rate*0.9)]
    res_df = res_df[(res_df['rotspd']>=speed_min*0.95) & (res_df['rotspd']<=speed_max*1.1)]
    ...
    res_df = res_df[(res_df['pitch1']<=2.0)]
    '''
...
def winddir_err_before_new(data,dirbin,windbin2,dirbin1,windbin1,path,turbine_name,request_num):
    ...
    if (np.max(yfit)-np.min(yfit))>0.35*(1 - np.cos(3.14159*(err_result_min - err_result)/180.0)):#0.3~0.4
    ...
    return err_result_get, str(path+'/'+str(turbine_name)+'_'+str(request_num)+'_yawerror.png')
...
#10min数据清洗
def data_min_clear_new(data,state,Rotspd_Connect,Rotspd_Rate,Pwrat_Rate,Pitch_Min=5,neighbors_num=20,threshold=3):  
    #data = data.dropna(axis=0,thresh=len(data.columns)*0.4)   
    data['clear'] = 10
    data['pitchlim'] = (data['pwrat','nanmean'] - 0.8*Pwrat_Rate)*(10.0-9-Pitch_Min) / (Pwrat_Rate*(0.95-0.8)) + (Pitch_Min+9.0)
    #风机发电状态
    data.loc[(data['state','nanmean']==state),'clear'] = 9
    
    #三一机组
    #data.loc[((data['state','nanmean']==state)|(data['state','nanmean']==120)),'clear'] = 9
    #风机转速、功率正常阈值内
    data.loc[(data['rotspd','nanmean']>Rotspd_Connect*0.9)&(data['rotspd','nanmean']<Rotspd_Rate*1.1)&(data['pwrat','nanmean']>0)&(data['pwrat','nanmean']<Pwrat_Rate*1.1)&(data['clear']==9),'clear'] = 8
    #非限功率状态
    data.loc[(data['pitch1','nanmean']>Pitch_Min)&(data['clear']==8),'clear'] = 7
    
    data.loc[((data['wspd','nanmean']<5)|((data['wspd','nanmean']>=5)&(data['pwrat','nanmean']>100)))&(data['clear']==7),'clear'] = 6
    
    #无超过正常范围的变桨
    data.loc[((data['pwrat','nanmean']>=Pwrat_Rate*0.95)|
              ((data['pwrat','nanmean']<Pwrat_Rate*0.8)&(data['pitch1','nanmean']<Pitch_Min+9.0))|
                  ((data['pwrat','nanmean']<Pwrat_Rate*0.95)&(data['pwrat','nanmean']>Pwrat_Rate*0.8)&(data['pitch1','nanmean']<data['pitchlim'])))&(data['clear']==6),'clear'] = 5
    
     
    #data_clear = thresholdfun_rotspd(data,neighbors_num,6)
    #data_clear = thresholdfun_pitch(data_clear,threshold,5)    
    data = thresholdfun_pwrat(data,threshold,5)
    #data = thresholdfun_wspd(data,Rotspd_Rate,threshold,4)
    #data_clear = thresholdfun_pwrat_out(data_clear,neighbors_num,2)
    return data
...
def data_min_clear(data,state,Rotspd_Connect,Rotspd_Rate,Pwrat_Rate,Pitch_Min,neighbors_num=20,threshold=3): 
    ...
    data = data[(data['pwrat','nanmean']>0)&(data['pwrat','nanmean']<Pwrat_Rate*1.1)]
    ...
    pwrat_range = np.arange(0,Pwrat_Rate*1.1+10,20.0)#######数据点太少容易出错
    pwrat_bin = np.arange(10,Pwrat_Rate*1.1,20.0)
    ...
    data.loc[(data['rotspd','nanmean']>Rotspd_Connect*0.9)&(data['rotspd','nanmean']<Rotspd_Rate*1.1)&(data['pwrat','nanmean']>0)&(data['pwrat','nanmean']<Pwrat_Rate*1.1)&(data['clear']==9),'clear'] = 8
    #非限功率状态
    data.loc[(data['statety','nanmean']==71)&(data['clear']==8),'clear'] = 7
...
def Pitch_Control_loss(data,Pitch_Min,Pwrat_Rate,Rotspd_Connect,Rotspd_Rate,mytol=0.1,myreg=0.02):           
    x_train = data['pwrat','nanmean'].values.reshape(-1,1)
    y_train = data['pitch1','nanmean'].values.reshape(-1,1)
    clf = GradientBoostingRegressor(n_estimators=20,random_state=0)
    clf.fit(x_train,y_train)
    
    x_test = np.arange(0.6,1.0+0.01,0.01)
...
def Turbine_Fault_Loss(data,turbine_name,pw_df_temp,fault_code):                   
    ...
    if len(data_fault) > 0: 
        ...
        data_fault.set_index(('localtime',''),inplace= True)
        data_fault = data_fault[((data_fault['statety','mymode']==30)|(data_fault['type']=='故障停机'))]
...
def Grid_Limit_Loss(data,turbine_name,pw_df_temp,fault_code): 
    ...
    data_limgrid = data_limgrid[(data_limgrid['statety','mymode']==80)|(data_limgrid['type']=='电网限电')]
...
###单机计划停机损失
def Stop_Loss(data,turbine_name,pw_df_temp,fault_code):     
    stop_loss = pd.DataFrame()
    data_stop = pd.DataFrame()
    temp = pw_df_temp[pw_df_temp['pwrat']>=50.0]
    ...
    data_stop = data_stop[(data_stop['statety','mymode']==10)|(data_stop['statety','mymode']==20)|          ##统一状态计划停机信息
                          ...
                          (data_stop['fault','nanmean']==0)&(data_stop['pitch1','nanmean']>=85)&(data_stop['wspd','nanmean']>=np.min(temp['windbin']))]  ###其它自行判断的计划停机信息
...
#单机电网故障损失
def Grid_Fault_Loss(data,turbine_name,pw_df_temp,fault_code):
    ...
    if len(data_grid) > 0: 
        ...
        data_grid.set_index(('localtime',''),inplace= True)
        data_grid = data_grid[(data_grid['statety','mymode']==40)|(data_grid['type']=='电网故障')]
...
#单机技术待命损失
def Turbine_Technology_Loss(data,turbine_name,pw_df_temp,fault_code):
...
##单机自限电损失输入
def Turbine_Limit_Loss(data,turbine_name,pw_df_temp,Pitch_Min,Pwrat_Rate,state): 
    ...
    #单层列名索引转多层列名索引
    columnList = []
    for elemColumn in pw_df_lim.columns.to_list():
        columnList.append((elemColumn, ''))
    pw_df_lim.columns = pd.MultiIndex.from_tuples(columnList)
    ...
    '''
    data_limt_lim = data_limt[(data_limt['pwrat','nanmean']<data_limt['pwrat_lim'])&(data_limt['pwrat','nanmean']<Pwrat_Rate*0.8)&(data_limt['pwrat','nanmean']>100.0)&(data_limt['pitch1','nanmin']>Pitch_Min+2.5)]
...
##时间可利用率计算(只考虑发电时间，包括正常发电和非正常发电)
def Time_Avail(data,wtids):
    temp = data[(data['pwrat','nanmean']>0)|(data['fault','mymode']==0)]
    #columns_index = temp.columns.get_loc(('pwrat','nanmean'))
    temp = temp.dropna(subset=[('pwrat','nanmean')])
    time_avail = len(temp)*10*60/len(wtids)/(np.max(temp.index) - np.min(temp.index)).total_seconds()
    #time_avail = len(temp)/len(data)
    return time_avail

#能量可利用率
def Eny_Avail(data,fault_loss,limgrid_loss,limturbine_loss,faultgrid_loss,stop_loss,Technology_loss):
    eny_avail = (np.nansum(data['pwrat','nanmean'])/6.0) / ((np.nansum(data['pwrat','nanmean'])/6.0)+fault_loss+faultgrid_loss+limgrid_loss+limturbine_loss+stop_loss+Technology_loss)  
    return eny_avail
        
#MTBT计算
def MTBT_Calculate(data,fault,wtids):
    if len(fault) > 0:
        fault_count = np.sum(fault['count'])
        MTBT = len(wtids) * (np.max(data.index) - np.min(data.index)).total_seconds() / 3600.0 /  fault_count
    else:
        MTBT = len(wtids) * (np.max(data.index) - np.min(data.index)).total_seconds() / 3600.0
    return MTBT
...
def abnormal_detect(data,turbine_camp,altitude,hub_high,rotor_radius,turbine_param_all,state,Pwrat_Rate,path,wtids_ses):
    ...
    for num in range(len(labelfen)):
        ...
        abnormal_detect = pd.concat([abnormal_detect, abnormal_detect_label])
    return abnormal_detect
        
# def Read_Data_From_zhongtai(Turbine_attr_type,day_range,Input_farmIds,Input_AccessKey,Input_SecretKey,Input_url_data,Input_pointIds0,Input_pointIds1,Input_pointIds2,Input_url_state,state):
#     ####单一机型机组
#     state_all_all = pd.DataFrame()
#     fault_all_all = pd.DataFrame()
#     Df_all_all = pd.DataFrame()
#     Df_all_m_all = pd.DataFrame()
#     tur_num = len(Turbine_attr_type)
#     warning_all = pd.DataFrame()
#     warning_all['warninge'] = str()

#     for num in range(tur_num):
#         restart = True
#         request_num = 0
#         while restart and request_num < 3:
#             Df_all = pd.DataFrame()
#             Df_all0 = pd.DataFrame()
#             Df_all1 = pd.DataFrame()
#             Df_all2 = pd.DataFrame()
#             state_all = pd.DataFrame()
#             state_all_CJ = pd.DataFrame()
#             state_all_TY = pd.DataFrame()
#             yaw_all = pd.DataFrame()
#             fault_all = pd.DataFrame()
#             Df_all_m = pd.DataFrame()
#             Input_assetIds = Turbine_attr_type.loc[num,'mdmId']
#             turbine_name = Turbine_attr_type.loc[num,'name']
#             warning_all.loc[num,'wtid'] = turbine_name
#             print(str(turbine_name+'---'+str(num)+'/'+str(tur_num)))
        
#             try:
#                 PoolNum = 24
#                 pnum = len(day_range)//PoolNum
#                 rem = len(day_range)%PoolNum
#                 if pnum > 0:       
#                     for i in range(pnum+1):
#                         try:
#                             pool = Pool()
#                             DaylistStart = day_range[i*PoolNum:(i+1)*PoolNum]
#                             data_result0 = pool.map_async(partial(ReadData_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,pointIds = Input_pointIds0,url = Input_url_data),DaylistStart)  #并行计算需执行py文件运行
#                             data_result1 = pool.map_async(partial(ReadData_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,pointIds = Input_pointIds1,url = Input_url_data),DaylistStart)  #并行计算需执行py文件运行
#                             data_result2 = pool.map_async(partial(ReadData_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,pointIds = Input_pointIds2,url = Input_url_data),DaylistStart)  #并行计算需执行py文件运行
#                             state_CJ_result = pool.map_async(partial(ReadState_CJ_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,url = Input_url_state),DaylistStart)
#                             state_TY_result = pool.map_async(partial(ReadState_TY_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,url = Input_url_state),DaylistStart)
#                             state_result = pool.map_async(partial(ReadState_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,url = Input_url_state),DaylistStart)
#                             yaw_result = pool.map_async(partial(ReadYaw_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,url = Input_url_state),DaylistStart)
#                             fault_result = pool.map_async(partial(ReadFault_Enos,AccessKey = Input_AccessKey,SecretKey = Input_SecretKey,assetIds = Input_assetIds,url = Input_url_state),DaylistStart)
#                             data_result0.get(timeout=300)
#                             data_result1.get(timeout=300)
#                             state_result.get(timeout=300)
#                             fault_result.get(timeout=300)
#                             pool.close()
#                             pool.join()
                            
                            
#                             datalist0 = data_result0.get()
#                             Df_temp0 = pd.concat(datalist0)
#                             # Df_all0 = Df_all0.append(Df_temp0)
#                             Df_all0 = pd.concat([Df_all0, Df_temp0])
#                             datalist1 = data_result1.get()
#                             Df_temp1 = pd.concat(datalist1)
#                             # Df_all1 = Df_all1.append(Df_temp1)
#                             Df_all1 = pd.concat([Df_all1, Df_temp1])
#                             datalist2 = data_result2.get()
#                             Df_temp2 = pd.concat(datalist2)
#                             # Df_all2 = Df_all2.append(Df_temp2)
#                             Df_all2 = pd.concat([Df_all2, Df_temp2])
                            
#                             yawlist = yaw_result.get()
#                             yaw_temp = pd.concat(yawlist)
#                             # yaw_all = yaw_all.append(yaw_temp)
#                             yaw_all = pd.concat([yaw_all, yaw_temp])
#                             statelist = state_result.get()
#                             state_temp = pd.concat(statelist)
#                             # state_all = state_all.append(state_temp)
#                             state_all = pd.concat([state_all, state_temp])
#                             statelist_CJ = state_CJ_result.get()
#                             state_temp_CJ = pd.concat(statelist_CJ)
#                             # state_all_CJ = state_all_CJ.append(state_temp_CJ)
#                             state_all_CJ = pd.concat([state_all_CJ, state_temp_CJ])
#                             statelist_TY = state_TY_result.get()
#                             state_temp_TY = pd.concat(statelist_TY)
#                             # state_all_TY = state_all_TY.append(state_temp_TY)
#                             state_all_TY = pd.concat([state_all_TY, state_temp_TY])
#                             faultlist = fault_result.get()
#                             fault_temp = pd.concat(faultlist)
#                             # fault_all = fault_all.append(fault_temp)
#                             fault_all = pd.concat([fault_all, fault_temp])
#                             text = str('进程数:'+str(i)+'/'+str(pnum)+';'+'数据长度'+str(len(Df_all1)))
#                             print(text)
#                         except TimeoutError:
#                             pool.terminate()
...
#                 print(str(1)) 
#                 if len(Df_all0) > 0:
#                     Df_all0['localtime'] = pd.to_datetime(Df_all0['localtime'],errors='coerce')    
#                     Df_all0.set_index('localtime',inplace= True)
#                     Df_all0.drop('timestamp',axis=1,inplace=True)
#                     Df_all0.drop('assetId',axis=1,inplace=True)
#                 if len(Df_all1) > 0:
#                     Df_all1['localtime'] = pd.to_datetime(Df_all1['localtime'],errors='coerce')    
#                     Df_all1.set_index('localtime',inplace= True)
#                     Df_all1.drop('timestamp',axis=1,inplace=True)
#                     Df_all1.drop('assetId',axis=1,inplace=True)
#                 if len(Df_all2) > 0:
#                     Df_all2['localtime'] = pd.to_datetime(Df_all2['localtime'],errors='coerce')    
#                     Df_all2.set_index('localtime',inplace= True)
#                     Df_all2.drop('timestamp',axis=1,inplace=True)
#                     Df_all2.drop('assetId',axis=1,inplace=True)
#                 print(str(2)) 
#                 if (len(Df_all0)>0) & (len(Df_all1)>0) & (len(Df_all1)>0):
#                     Df_all = pd.concat([Df_all0,Df_all1,Df_all2],axis=1)
#                 else:
#                     Df_all = pd.concat([Df_all0,Df_all1],axis=1)
#                 print(str(3)) 
...
#                 if len(state_all_CJ) > 0:
#                     state_all_CJ['localtime'] = pd.to_datetime(state_all_CJ['localtime'],errors='coerce')    
#                     state_all_CJ.set_index('localtime',inplace= True)
#                     state_all_CJ.drop('timestamp',axis=1,inplace=True)
#                     state_all_CJ.drop('assetId',axis=1,inplace=True)
#                     state_all_CJ.rename(columns = {'WTUR.TurbineSts':'state','WTUR.TurbineSts_Map':'state'},inplace = True)
#                     state_all_CJ = state_all_CJ.dropna(axis=1,how='all')
#                     state_all_CJ = state_all_CJ.loc[:,~state_all_CJ.columns.duplicated()]
                    
#                     state_all_CJ['sta1'] = state_all_CJ['state'].shift(periods=1,axis=0)
#                     state_all_CJ['shift'] = state_all_CJ['state']-state_all_CJ['sta1']
#                     state_all_CJf = state_all_CJ.loc[state_all_CJ['shift']!=0,'state']####剔除连续时间重复状态
#                     Df_all = Df_all.join(state_all_CJf,how='outer')
#                     Df_all['state'] = Df_all['state'].fillna(method='ffill')
#                 else:
#                     Df_all['state'] = state
                    
#                 print(str(6)) 
#                 if len(state_all_TY) > 0:
#                     state_all_TY['localtime'] = pd.to_datetime(state_all_TY['localtime'],errors='coerce')    
#                     state_all_TY.set_index('localtime',inplace= True)
#                     state_all_TY.drop('timestamp',axis=1,inplace=True)
#                     state_all_TY.drop('assetId',axis=1,inplace=True)
#                     state_all_TY.rename(columns = {'WTUR.TurbineUnionSts':'statety'},inplace = True)
                    
#                     state_all_TY['sta1'] = state_all_TY['statety'].shift(periods=1,axis=0)
#                     state_all_TY['shift'] = state_all_TY['statety']-state_all_TY['sta1']
#                     state_all_TYf = state_all_TY.loc[state_all_TY['shift']!=0,'statety']####剔除连续时间重复状态
#                     Df_all = Df_all.join(state_all_TYf,how='outer')
#                     Df_all['statety'] = Df_all['statety'].fillna(method='ffill')
#                 else:
#                     Df_all['statety'] = 71
                
                
#                 #yaw_all['localtime'] = pd.to_datetime(yaw_all['localtime'],errors='coerce')    
#                 #yaw_all.set_index('localtime',inplace= True)
#                 #yaw_all.drop('timestamp',axis=1,inplace=True)
#                 #yaw_all.drop('assetId',axis=1,inplace=True)
#                 #yaw_all.rename(columns = {'WYAW.YawCCWSts':'yawsssta','WYAW.YawCWSts':'yawnssta'},inplace = True)
    
#                 print(str(6)) 
#                 if len(fault_all) > 0:
#                     fault_all['localtime'] = pd.to_datetime(fault_all['localtime'],errors='coerce')    
...
#                 print(str(8)) 
#                 state_all.insert(0,'wtid',turbine_name)
#                 # state_all_all = state_all_all.append(state_all)
#                 state_all_all = pd.concat([state_all_all, state_all])
#                 fault_all.insert(0,'wtid',turbine_name)
#                 # fault_all_all = fault_all_all.append(fault_all)
#                 fault_all_all = pd.concat([fault_all_all, fault_all])
...
#                 restart = False                          
#             except Exception:
#                 warning_text = str(turbine_name + '数据获取失败,重新读取')
#                 print(warning_text)
#                 errorInfomation = traceback.format_exc()
#                 print(f'\033[31m{errorInfomation}\033[0m')
#                 warning_all.loc[num,'warninge'] = warning_text
#                 request_num = request_num + 1
#     return Df_all_all,Df_all_m_all



'''

temp = data[(data['state','nanmean']==state)&(data['pwrat','nanmean']>0)&(data['pwrat','nanmean']<Pwrat_Rate*0.6)]   

fig = plt.figure(figsize=(10,8),dpi=100)  
plt.title(str(turbine_name))    
with plt.style.context('ggplot'):  
    plt.scatter(Df_all_m['wspd','nanmean'],Df_all_m['pwrat','nanmean'],c=Df_all_m['statel','nanmean'],cmap='jet',s=15)
    plt.grid()
    #plt.ylim(-3,25)
    plt.xlabel('风速(m/s)',fontsize=14)
    plt.ylabel('功率(kW)',fontsize=14)
    plt.colorbar()
    plt.colorbar(label='月份',ticks=np.unique(df_all_clear.index.month)) 
    


Df_all_m = Df_all_hadian[Df_all_hadian['wtid']=='A26']   
data = Df_all_m 
df_all_clear = data_clear[data_clear['clear']==2]
pw_df = pwratcurve_rho(df_all_clear,windbin,6)  

with plt.style.context('ggplot'):  
    #plt.plot(biaozhun['wspd'],biaozhun['pwrat'],color='red')          
    plt.plot(pw_df['windbin'],pw_df['pwrat'],'-o',color='black',markersize=5,label='全场平均')
