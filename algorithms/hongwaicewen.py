...
name = algConfig['hongwaicewen']['name']#'红外测温'
# 把所需测点定义到每个算法里
ai_points = algConfig['hongwaicewen']['ai_points']
ai_rename = algConfig['hongwaicewen']['ai_rename']
di_points = algConfig['hongwaicewen']['di_points']
general_points = algConfig['hongwaicewen']['general_points']
private_points = algConfig['hongwaicewen']['private_points']
# modelId = algConfig['hongwaicewen']['name']
time_duration = algConfig['hongwaicewen']['time_duration']
resample_interval = algConfig['hongwaicewen']['resample_interval']
error_data_time_duration = algConfig['hongwaicewen']['error_data_time_duration']
horizonTime = algConfig['hongwaicewen']['horizonTime']
need_all_turbines = algConfig['hongwaicewen']['need_all_turbines']
input_chunk_length = algConfig['hongwaicewen']['input_chunk_length']
out_chunck_length = algConfig['hongwaicewen']['out_chunck_length']
futureCovariates = algConfig['hongwaicewen']['futureCovariates']
numEpoch = algConfig['hongwaicewen']['numEpoch']
trainCutOff = algConfig['hongwaicewen']['trainCutOff']
timeSampleNum = algConfig['hongwaicewen']['timeSampleNum']
store_file = algConfig['hongwaicewen']['store_file']
timeUnit = resample_interval #'10T' #10minute: 10T,  1day:1D, 1quater:1Q
forcastHorizon = out_chunck_length
...
def wash_data(pn_data: DataFrame, ratedPower):
    ...
    if not temp_data.empty:
        ...
        return temp_data, f'{private_points_}数据没有'

def predict_result(pn_data: DataFrame):

    return pn_data[['LOW_A_TMP']].dropna()
...
async def judge_model(pn_data: DataFrame, Turbine_attr, threshold, idMaps,algorithms_config):
    ...
    alarming = 0 #告警
    warning = 0 #预警
    measureName = 'LOW_A_TMP'
    
    #告警
    statementException = f'温度超过阈值{threshold["gaojing"]["threshold"]["10"]}，'
    statementNormal = f'温度未超过阈值{threshold["gaojing"]["threshold"]["10"]}'
    # 生成告警
    final_df['result'] = final_df[measureName] > threshold["gaojing"]["threshold"]["10"]
    data, statement, alarming =  alarm.generateAlarm(name,'hongwaicewen','LOW_A_TMP', final_df, error_data_time_duration, resample_interval, assetId, threshold, statementException, statementNormal, idMaps)
    if data.empty == False:
        # 展示数据
        ...
        Figs = []
        curves1 = []
        curves1.append(DisplayResultXY('0', '温度', '#FFFF00', 'Solid', data1))#{'type':'0', 'name':'倾角', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data1}
        ...
        result1 = DisplayFigures(xUnit=interval_unit, yUnit="温度[℃]", time=1, multiDimensionDataxy=curves1)#DisplayResultXY(str(final_df.index.min()), str(final_df.index.max()), str(interval_value), '角度', curves)
        Figs.append(result1)
        return data, statement, Figs, int(alarming), int(warning)
    elif data.empty == True and ("minute" not in threshold["executeTimeValue"] and  "hour" not in threshold["executeTimeValue"]): #and len(threshold["executeTimeValue"])!=0
        data1, data2, statement, warning, startTime, endTime = alarm.generateAlarmTrend(name, final_df, measureName, threshold, resample_interval, assetId, horizonTime)
        # 展示数据
        interval_value, interval_unit = time_util.split_time_delta(resample_interval) 
        interval_unit = time_util.__timedelta_resample_unit_dict[interval_unit].lower()
        ...
        curves1 = []
        curves1.append(DisplayResultXY('0', '温度', '#FFFF00', 'Solid', data1))#{'type':'0', 'name':'倾角', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data1}
        curves1.append(DisplayResultXY('0', '预警线', '#00FF00', 'Solid', data2))#{'type':'0', 'name': '告警线', "abscissaUnit": interval_unit, "ordinateUnit": "°", 'xyData': data2}
...
#     #取平均
#     series = MeanOverTime(final_df['LOW_A_TMP'], timeUnit)
#     # #归一化
...
#     iouList = []
#     disRateList = []
#     timeDelta = pd.Timedelta(timeSampleNum*4*int(interval_value), unit=time_util.__timedelta_resample_unit_dict[interval_unit])
#     for countIndex, time_index in enumerate(date_rng):
#         startTime = time_index #+ pd.Timedelta(timeSampleNum//2, unit=timeUnit)
#         endTime = time_index + timeDelta#+ pd.Timedelta(timeSampleNum//2, unit=timeUnit)
#         # if countIndex == 0:
...
#                 counts = final_df.loc[startTime:,'result'].iloc[:freqValue].shape[0]
#         else:
#             if tftModel.backTestSeries.time_index[-1] > endTime:
...
#     # final_df = predict_result(pn_data)
#     #异常为True
#     # final_df['result'] = (np.abs(final_df['WROT.Blade1Position'] - final_df['WROT.Blade2Position']) >=1) | (np.abs(final_df['WROT.Blade1Position'] - final_df['WROT.Blade3Position']) >=1) | (np.abs(final_df['WROT.Blade3Position'] - final_df['WROT.Blade2Position']) >=1)
...
#     y1 = [str(round(num,4)) for num in series_new[:len(series_new)]]
...
#     interval_unit = time_util.__timedelta_resample_unit_dict[interval_unit].lower()
#     curves = []
#     curves.append({'type':'0', 'name': '原始温度', "abscissaUnit": interval_unit, "ordinateUnit": "°C", 'xyData': data1})
#     curves.append({'type':'0', 'name': '温度上限', "abscissaUnit": interval_unit, "ordinateUnit": "°C", 'xyData': data2})
#     curves.append({'type':'0', 'name': '温度下限', "abscissaUnit": interval_unit, "ordinateUnit": "°C", 'xyData': data3})
...
#     result = DisplayResultXY(series.time_index[input_chunk_length], series.time_index.max(), str(interval_value), '温度', curves)

#     statementException = f'跟据当前温度变化, 存在相邻时间窗口波动中心距超高{0.5}倍波动区间且相邻波动区间IOU小于{0.5}'
#     statementNormal = f'跟据当前温度变化, 不存在相邻时间窗口波动中心距超高{0.5}倍波动区间且相邻波动区间IOU小于{0.5}'
#     # 生成告警
