...
async def judge_model(pn_data: DataFrame, Turbine_attr, threshold, idMaps,algorithms_config):
    ...
    # 平均温度-拟合温度
    pn_data['result'] = np.abs(np.mean(pn_data[['WGEN.TemGenStaU','WGEN.TemGenStaV','WGEN.TemGenStaW']], axis=1) - predict) > error*2# | (np.abs(np.mean(pn_data[['WGEN.TemGenStaU','WGEN.TemGenStaV','WGEN.TemGenStaW']], axis=1) - predict) < error*-3)
    ...
    statementException = f'线圈三相平均温度在正常工况下给出的预测和当前实际温度差值大于3倍标准差'
    statementNormal = f'线圈三相平均温度在正常工况下给出的预测和当前实际温度差值小于等于3倍标准差'
    # 生成告警
