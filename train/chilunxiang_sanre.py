...
async def main():
    ...
    for index, row in df_wind_turbine.iterrows():
        ...
        if os.path.exists(os.path.join('model',config.Wind_Farm, name, assetId, 'chilunxiang_sanre.model')):
            continue
        ...
        import joblib
        if os.path.exists(os.path.dirname(os.path.join('model',config.Wind_Farm, name, assetId, 'chilunxiang_sanre.model'))):
            pass
        else:
            os.makedirs(os.path.dirname(os.path.join('model',config.Wind_Farm, name, assetId, 'chilunxiang_sanre.model')))
        joblib.dump(gs.best_estimator_, os.path.join('model',config.Wind_Farm, name, assetId, 'chilunxiang_sanre.model'))
