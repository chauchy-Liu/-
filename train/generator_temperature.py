...
async def main():
    ...
    for index, row in df_wind_turbine.iterrows():
        ...
        if os.path.exists(os.path.join('model',config.Wind_Farm, name, assetId, 'generator_temperature.model')):
            continue
        ...
        final_df = final_df.dropna(subset=['mean_tem', 'WGEN.GenActivePW', 'WNAC.TemOut','WNAC.TemNacelle','WGEN.GenSpd','WNAC.WindSpeed'])
        X = final_df[['WGEN.GenActivePW','WNAC.TemOut','WNAC.TemNacelle','WGEN.GenSpd','WNAC.WindSpeed']] # 'WGEN.LHDLGENAI31','WGEN.LHDLGENAI103'
        y = final_df['mean_tem']
        ...
        import joblib
        if os.path.exists(os.path.dirname(os.path.join('model',config.Wind_Farm, name, assetId, 'generator_temperature.model'))):
            pass
        else:
            os.makedirs(os.path.dirname(os.path.join('model',config.Wind_Farm, name, assetId, 'generator_temperature.model')))
        joblib.dump(gs.best_estimator_, os.path.join('model',config.Wind_Farm, name, assetId, 'generator_temperature.model'))
